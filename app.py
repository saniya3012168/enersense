...
# ---------------- Grid Optimization ----------------
@app.route('/grid-optimization')
def grid_optimization():
    regions = ["North", "South", "East", "West"]
    allocations = [50, 70, 40, 90]  # Example MW allocations
    return render_template("grid_optimization.html", regions=regions, allocations=allocations)

# ---------------- Equity ----------------
@app.route('/equity')
def equity():
    communities = ["Urban", "Rural", "Remote"]
    access_levels = [90, 60, 45]  # Example %
    return render_template("equity.html", communities=communities, access_levels=access_levels)

# ---------------- Clean Energy Adoption ----------------
@app.route('/clean-energy')
def clean_energy():
    energy_sources = ["Solar", "Wind", "Hydro", "Biomass"]
    adoption_rates = [65, 50, 40, 25]  # Example %
    return render_template("clean_energy.html", energy_sources=energy_sources, adoption_rates=adoption_rates)
